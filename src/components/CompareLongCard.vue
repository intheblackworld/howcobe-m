  ...
  <div :class="`compare-long ${dark ? 'dark-card' : ''}`">
    <div class="compare-card flex-ac flex-jb" v-if="compare.courses.length > 0">
      ...
      <div class="action">
        ...
        <div class="pink round-big middle btn flex-c" @click="addCompareToCollect(compare.compare_course_id)">加入收藏</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addCompareToCollect(compare_course_id) {
      addCollectCompare(compare_course_id).then(() => {
        this.$message({
          message: '已收藏此列表',
          type: 'success',
        })
